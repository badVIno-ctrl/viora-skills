...
Suppression: if a ban genuinely does not apply (a neo-brutalist direction that wants hard shadows, an editorial piece that wants section numbers), add `/* viora-allow: RULE-ID reason */` next to the code so the decision is visible and the checker stays quiet.
