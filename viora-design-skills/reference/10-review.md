...
- **Every WARN gets a decision.** Either fix it, or add `/* viora-allow: rule-id reason */` on the line above so the choice is recorded. Silently ignoring warnings is how quality drifts.
...
```bash
node <skill-path>/scripts/shot.mjs http://localhost:3000 --out .viora-shots
