...
LITE records four gates, not eight, and `gate.mjs` holds a LITE run to exactly these.
One command each, and only if `node` runs:
...
The lane argument matters: `LITE` is what tells the verifier to ask for these four and not
for the eight the FULL lane owes.
