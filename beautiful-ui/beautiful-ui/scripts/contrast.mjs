...
if (pairIndex !== -1) {
	...
	if (!a || !b) {
		console.error(`beautiful-ui contrast: cannot parse ${!a ? fg : bg}`)
		process.exit(2)
	}
...
if (!target) {
	console.error("beautiful-ui contrast: pass a token file, or --pair <fg> <bg>")
	process.exit(2)
}
if (!existsSync(target)) {
	console.error(`beautiful-ui contrast: file not found: ${target}`)
	process.exit(2)
}
...
if (rows.length === 0) {
	console.log(
		"beautiful-ui contrast: no known token names found. Expected --ink, --canvas,\n" +
...
console.log("\n" + bar)
console.log(
	`beautiful-ui contrast: ${failures.length} failure${failures.length === 1 ? "" : "s"}` +
