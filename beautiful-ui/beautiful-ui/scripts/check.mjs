#!/usr/bin/env node
/**
 * beautiful-ui / mechanical craft checker
 *
 ...
 *   /* bui-allow: rule-id short reason *\/
 *
...
const SKIP_DIRS = new Set([
	...
	"vendor", "target", "__pycache__", ".venv", "venv", ".bui-shots",
])
...
for (const t of targets) {
	const p = resolve(t)
	if (!existsSync(p)) {
		console.error(`beautiful-ui: path not found: ${t}`)
		process.exit(2)
	}
...
for (const file of files) {
	...
	const allowed = rawLines.map((l) => {
		const m = l.match(/bui-allow:\s*([a-z0-9-,*\s]+)/i)
		if (!m) return null
...
console.log("\n" + bar)
console.log(
	`beautiful-ui: ${errors.length} error${errors.length === 1 ? "" : "s"}, ` +
...
if (errors.length === 0 && warns.length === 0) {
	...
	console.log("a reason: /* bui-allow: rule-id why this is correct here */")
}
console.log(bar)
